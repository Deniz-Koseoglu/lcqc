...
addpars <- params$addpars
mainmets <- c(mainmets[addpars$nametag], lapply(mainmets[!names(mainmets) %in% addpars$nametag], as.numeric))
spec <- params$spec
...
\cohead{\includegraphics[width=6cm]{"`r if(logocond) noquote("img/custom_logo.jpg") else noquote("img/sample_logo.jpg")`"}}

<!-- \raisebox{\depth}{\leftmark}\hfill -->

\rohead{\usekomafont{pagefoot}{`r tpars[["tform"]]` \\Оператор: `r tpars[["oper"]]`}} 
\lohead{Отчёт о производительности\\ ЖХ-колонки} 
\lofoot{Колонка поставляется в:\\ `r tpars[["sp"]]`.}

<!-- [footheight=30pt] -->

\rofoot{Номер документа:\\ `r tpars[["dnum"]]`}
...
::: {#third layout-valign = "top"}
<!-- \vspace{-72pt} -->
```{r, ft.align = "left", ft.valign = "top", ft.arraystretch = 1.3}
#| layout-valign: bottom
prodtbl <- tribble(~col1, ~col2,
               "Серийный номер:", tpars[["sn"]],
               "Номер детали:", tpars[["pn"]],
               "Описание:", tpars[["desc"]],
               "Номер партии:", tpars[["bn"]])
flextable(prodtbl) |>
  delete_part("header") |>
  border_remove() |>
  width(1,1.5) |>
...
::: {#fourth layout-valign = "top"}
<!-- \vspace{-13pt} -->
```{r, ft.align = "left", ft.valign = "top", ft.arraystretch = 1.3}
#| layout-valign: bottom
mtbl <- tribble(~col1, ~col2,
               "Подвижная фаза:", tpars[["mp"]],
               "Обратное давление:", tpars[["bp"]],
               "Скорость потока:", tpars[["flow"]],
               "Температура:", tpars[["temp"]],
               "Объём инжекции:", tpars[["inj"]],
               "Тестовая смесь:", tpars[["tmix"]])
flextable(mtbl) |>
  delete_part("header") |>
  border_remove() |>
  width(1,1.5) |>
...
```{r ft.align = "left", ft.valign = "top", ft.arraystretch = 1.1}
...
## Специфические метрики колонки

\vspace{-10pt}

```{r}
if (!excond) { # Only execute if extmets are provided
...
layvec <- sapply(layvec, function(x) {
  ...
  res <- paste0("[", paste0(rep(1,len),collapse = ","), "]")
  return(res)})
...
#Includes an initial whole-width panel (for title) and final (for plot whitespace and TPA plot legend)
```
...
```{r}
#| output: asis
tpath_fin <- paste0("img/",c(unlist(params$tpath), "whitespace.png"))
if(ceiling(length(tpath_fin))%%2 != 0) tpath_fin <- tpath_fin[-length(tpath_fin)]
open <- "\n:::{}\n"
close <- "\n:::\n"
dl <- "<<"
dr <- ">>"
...
::: {#secondtolast}
![](img/tpa_leg.png){width="100%" fig-align="center"}
:::
